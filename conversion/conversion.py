...
from nffg_lib import NFFG
from nffg_lib.nffg_elements import NodeSAP
...
def convert_service_request (request, base, reinstall=False, log=None):
  """
  Wrapper function to convert service request on-the-fly.
  ...
  :rtype :class:`Virtualizer`
  """
  logger_name = "NFFGConverter"
  log = log.getChild(logger_name) if log else logging.getLogger(logger_name)
  converter = NFFGConverter(logger=log, ensure_unique_id=False)
  return converter.convert_service_request(request=request,
                                           base=base,
                                           reinstall=reinstall)
